...
function createCircularGrid(scene: THREE.Scene): void {
  ...
  for (let i = 1; i <= ringCount; i++) {
    ...
    const ringGeometry = new THREE.RingGeometry(radius - 0.05, radius, segments);
  ...
  for (let i = 0; i < lineCount; i++) {
    ...
    const vertices = [
      0, 0, -5, // Start at center
      Math.cos(angle) * lineLength, Math.sin(angle) * lineLength, -5 // End at edge
    ];
