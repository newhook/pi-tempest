...
function createCircleLevel(group: THREE.Group, radius: number): void {
  ...
  spokeGeometry.setAttribute('position', new THREE.Float32BufferAttribute(positions, 3));
...
function createSpiralLevel(group: THREE.Group, radius: number): void {
  ...
  spiralGeometry.setAttribute('position', new THREE.Float32BufferAttribute(points, 3));
...
function createStarLevel(group: THREE.Group, radius: number, points: number): void {
  // Create a star shape
...
function createPiSymbolLevel(group: THREE.Group, radius: number): void {
  ...
  const piVertices = [
    // Top horizontal line
    -0.6 * scale, 0.5 * scale, 0,
    0.6 * scale, 0.5 * scale, 0,
    
    // Left vertical line
    -0.4 * scale, 0.5 * scale, 0,
    -0.4 * scale, -0.5 * scale, 0,
    
    // Right vertical line
    0.4 * scale, 0.5 * scale, 0,
    0.4 * scale, -0.3 * scale, 0
  ];
  
  piGeometry.setAttribute('position', new THREE.Float32BufferAttribute(piVertices, 3));
...
function createWaveLevel(group: THREE.Group, radius: number): void {
  ...
  for (let r = radius * 0.2; r < radius; r += radius * 0.2) {
    ...
    waveGeometry.setAttribute('position', new THREE.Float32BufferAttribute(points, 3));
