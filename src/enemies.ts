...
class EnemyExplosion {
  ...
  constructor(scene: THREE.Scene, position: THREE.Vector3, color: THREE.Color, modeState: ActiveModeState) {
    this.scene = scene;
  ...
  private animateRings(progress: number): void {
    ...
    for (let i = 0; i < this.rings.length; i++) {
      ...
      const ringProgress = Math.max(0, Math.min(1, (progress - ringDelay) * 1.5)); // Adjusted progress for this ring
  ...
  private updateExplosionRadius(progress: number): void {
    ...
    const explosionIndex = this.modeState.explosions.findIndex(exp => exp.id === this.explosionId);
